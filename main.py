...
class BREadbeatsWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setMinimumSize(400, 300)  # Allow shrinking to miniplayer
        self.resize(1100, 950)  # Larger default for proper visualizer rendering
        self.setStyleSheet(self._get_stylesheet())
        
        # Miniplayer mode tracking
        self._miniplayer_mode = False
        self._miniplayer_threshold = 700  # Width threshold for miniplayer
    ...
    def _setup_ui(self):
        ...
        # Top: Connection and controls (hideable in miniplayer)
        self._top_widget = QWidget()
        top_layout = QHBoxLayout(self._top_widget)
        top_layout.setContentsMargins(0, 0, 0, 0)
        self._connection_panel = self._create_connection_panel()
        self._control_panel = self._create_control_panel()
        top_layout.addWidget(self._connection_panel)
        top_layout.addWidget(self._control_panel)
        self._main_layout.addWidget(self._top_widget)
        
        # Middle: Visualizers (layout changes in miniplayer)
        self._viz_widget = QWidget()
        self._viz_layout = QHBoxLayout(self._viz_widget)
        self._viz_layout.setContentsMargins(0, 0, 0, 0)
        self._spectrum_panel = self._create_spectrum_panel()
        self._position_panel = self._create_position_panel()
        self._viz_layout.addWidget(self._spectrum_panel, stretch=3)
        self._viz_layout.addWidget(self._position_panel, stretch=1)
        self._main_layout.addWidget(self._viz_widget)
        
        # Bottom: Tabs with sliders (hideable in miniplayer)
        self._settings_tabs = self._create_settings_tabs()
        self._main_layout.addWidget(self._settings_tabs)
        
        # Bottom row: Presets + Visualizer controls (hideable in miniplayer)
        self._bottom_widget = QWidget()
        bottom_layout = QHBoxLayout(self._bottom_widget)
        bottom_layout.setContentsMargins(0, 0, 0, 0)
        self._presets_panel = self._create_presets_panel()
        bottom_layout.addWidget(self._presets_panel)
        
        # Visualizer controls in a groupbox
        self._visualizer_group = QGroupBox("Spectrum")
        visualizer_layout = QHBoxLayout(self._visualizer_group)
        ...
        bottom_layout.addWidget(self._visualizer_group)
